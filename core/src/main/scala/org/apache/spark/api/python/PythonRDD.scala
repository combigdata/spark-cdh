  ...
  class WriterThread(
    ...
    override def run(): Unit = Utils.logUncaughtExceptions {
      try {
        ...
        val needsDecryptionServer = env.securityManager.getIOEncryptionKey().isDefined &&
          addedBids.nonEmpty
        ...
        if (needsDecryptionServer) {
          ...
          val idsAndFiles = broadcastVars.asScala.flatMap { broadcast =>
  ...
  def setupEncryptionServer(): Array[Any] = {
    encryptionServer = new PythonServer[Unit]("broadcast-encrypt-server") {
      override def handleConnection(sock: Socket): Unit = {
        ...
        val file = File.createTempFile("broadcast", "", dir)
        path = file.getAbsolutePath
        val out = CryptoStreamUtils.wrapForEncryption(new FileOutputStream(path), env.conf)
  ...
  override def handleConnection(socket: Socket): Unit = {
    ...
    Utils.tryWithSafeFinally {
      (idsAndFiles).foreach { case (id, path) =>
        out.writeLong(id)
        val in = CryptoStreamUtils.wrapForEncryption(new FileInputStream(path), env.conf)
        Utils.tryWithSafeFinally {
