...
private[spark] class CoarseMesosSchedulerBackend(
  ...
  def createCommand(offer: Offer, numCores: Int): CommandInfo = {
    ...
    if (uri == null) {
      ...
      command.setValue(
        "%s \"%s\" org.apache.spark.executor.CoarseGrainedExecutorBackend %s %s %s %d".format(
          prefixEnv, runScript, driverUrl, offer.getSlaveId.getValue, offer.getHostname, numCores))
    } else {
