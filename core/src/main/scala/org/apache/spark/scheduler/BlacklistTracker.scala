...
private[scheduler] class BlacklistTracker (
  ...
  private val BLACKLIST_FETCH_FAILURE_ENABLED =
    conf.getBoolean(BlacklistConfs.BLACKLIST_FETCH_FAILURE_ENABLED, false)
      ...
      allocationClient match {
        case Some(a) =>
          logInfo(s"Killing blacklisted executor id $exec " +
            s"since ${BlacklistConfs.BLACKLIST_KILL_ENABLED} is set.")
...
private[spark] object BlacklistConfs {
  ...
  val BLACKLIST_FETCH_FAILURE_ENABLED = "spark.blacklist.application.fetchFailure.enabled"
}
